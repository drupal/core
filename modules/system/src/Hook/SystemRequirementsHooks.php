...
class SystemRequirementsHooks {
  ...
  public function updateRequirements(): array {
    return $this->checkRequirements('update');
  }
   ...
   *   hook_runtime_requirements() and hook_update_requirements().
   */
  public function checkRequirements(string $phase): array {
    ...
    if ($phase == 'runtime') {
      ...
      if ($profile != 'standard' && !empty($profile)) {
        ...
        $requirements['install_profile'] = [
          ...
          'value' => $this->t('%profile_name (%profile%version)', [
      ...
      if (!empty($experimental_modules)) {
        $requirements['experimental_modules'] = [
          'title' => $this->t('Experimental modules installed'),
          'value' => $this->t('Experimental modules found: %module_list. <a href=":url">Experimental modules</a> are provided for testing purposes only. Use at your own risk.',
            [
      ...
      if (!empty($deprecated_modules)) {
        ...
        $requirements['deprecated_modules'] = [
          'title' => $this->t('Deprecated modules installed'),
          'value' => $this->t('Deprecated modules found: %module_list.', [
      ...
      if (!empty($deprecated_themes)) {
        ...
        $requirements['deprecated_themes'] = [
          'title' => $this->t('Deprecated themes installed'),
          'value' => $this->t('Deprecated themes found: %theme_list.', [
      ...
      if (!empty($obsolete_extensions)) {
        ...
        $requirements['obsolete_extensions'] = [
          'title' => $this->t('Obsolete extensions installed'),
          'value' => $this->t('Obsolete extensions found: %extensions. Obsolete extensions are provided only so that they can be uninstalled cleanly. You should immediately <a href=":uninstall_url">uninstall these extensions</a> since they may be removed in a future release.', [
      ...
      SystemRequirements::systemAdvisoriesRequirements($requirements);
    }
    ...
    if ($phase === 'runtime') {
      $phpversion_label = $this->t('@phpversion (<a href=":url">more information</a>)', [
    ...
    if (version_compare($phpversion, $minimum_supported_php) < 0) {
      $requirements['php']['description'] = $this->t(
    ...
    elseif ($phase === 'runtime' && version_compare($phpversion, \Drupal::RECOMMENDED_PHP) < 0) {
      $requirements['php']['description'] = $this->t('It is recommended to upgrade to PHP version %recommended or higher for the best ongoing support.  See <a href="http://php.net/supported-versions.php">PHP\'s version support documentation</a> and the <a href=":php_requirements">Drupal PHP requirements</a> page for more information.',
        [
          '%recommended' => \Drupal::RECOMMENDED_PHP,
          ':php_requirements' => 'https://www.drupal.org/docs/system-requirements/php-requirements',
        ]
    ...
    if (!empty($missing_extensions)) {
      $description = $this->t('Drupal requires you to enable the PHP extensions in the following list (see the <a href=":system_requirements">system requirements page</a> for more information):', [
      ...
      $requirements['php_extensions']['value'] = $this->t('Disabled');
      $requirements['php_extensions']['severity'] = RequirementSeverity::Error;
      $requirements['php_extensions']['description'] = $description;
    }
    else {
      $requirements['php_extensions']['value'] = $this->t('Enabled');
    }
    ...
    if ($phase == 'runtime') {
      ...
      if (!OpCodeCache::isEnabled()) {
        $requirements['php_opcache'] = [
          ...
          'severity' => RequirementSeverity::Warning,
          'description' => $this->t('PHP OPcode caching can improve your site\'s performance considerably. It is <strong>highly recommended</strong> to have <a href="http://php.net/manual/opcache.installation.php" target="_blank">OPcache</a> installed on your server.'),
        ];
      }
      else {
        $requirements['php_opcache']['value'] = $this->t('Enabled');
      }
      $requirements['php_opcache']['title'] = $this->t('PHP OPcode caching');
    }

    // Check to see if APCu is installed and configured correctly.
    if ($phase == 'runtime' && PHP_SAPI != 'cli') {
      $requirements['php_apcu_enabled']['title'] = $this->t('PHP APCu caching');
      $requirements['php_apcu_available']['title'] = $this->t('PHP APCu available caching');
      if (extension_loaded('apcu') && filter_var(ini_get('apc.enabled'), FILTER_VALIDATE_BOOLEAN)) {
        ...
        if ((Bytes::toNumber(ini_get('apc.shm_size')) * $apcu_shm_segments) < Bytes::toNumber($apcu_recommended_size)) {
          ...
          $requirements['php_apcu_enabled']['description'] = $this->t('Depending on your configuration, Drupal can run with a @apcu_size APCu limit. However, a @apcu_default_size APCu limit (the default) or above is recommended, especially if your site uses additional custom or contributed modules.', [
          ...
          if ($memory_available < 0.1) {
            ...
            $requirements['php_apcu_available']['description'] = $this->t('APCu is using over 90% of its allotted memory (@apcu_actual_size). To improve APCu performance, consider increasing this limit.', [
          ...
          elseif ($memory_available < 0.25) {
            ...
            $requirements['php_apcu_available']['description'] = $this->t('APCu is using over 75% of its allotted memory (@apcu_actual_size). To improve APCu performance, consider increasing this limit.', [
              '@apcu_actual_size' => $apcu_actual_size,
            ]);
          }
          else {
        ...
        $requirements['php_apcu_enabled'] += [
          'value' => $this->t('Not enabled'),
          'severity' => RequirementSeverity::Info,
          'description' => $this->t('PHP APCu caching can improve your site\'s performance considerably. It is <strong>highly recommended</strong> to have <a href="https://www.php.net/manual/apcu.installation.php" target="_blank">APCu</a> installed on your server.'),
        ];
      }
    }
    ...
    if ($phase == 'runtime') {
      ...
      $requirements['php_random_bytes'] = [
        'title' => $this->t('Random number generation'),
      ];
      try {
        ...
        $requirements['php_random_bytes']['value'] = $this->t('Successful');
      }
      catch (\Exception $e) {
        ...
        if ($open_basedir_blocks_urandom) {
          $requirements['php_random_bytes']['description'] = $this->t('Drupal is unable to generate highly randomized numbers, which means certain security features like password reset URLs are not as secure as they should be. Instead, only a slow, less-secure fallback generator is available. The most likely cause is that open_basedir restrictions are in effect and /dev/urandom is not on the allowed list. See the <a href=":drupal-php">system requirements</a> page for more information. %exception_message', $args);
        }
        else {
          $requirements['php_random_bytes']['description'] = $this->t('Drupal is unable to generate highly randomized numbers, which means certain security features like password reset URLs are not as secure as they should be. Instead, only a slow, less-secure fallback generator is available. See the <a href=":drupal-php">system requirements</a> page for more information. %exception_message', $args);
        }
        $requirements['php_random_bytes']['value'] = $this->t('Less secure');
    ...
    if ($phase === 'runtime' && PHP_SAPI !== 'cli') {
      if (!function_exists('fastcgi_finish_request') && !function_exists('litespeed_finish_request') && !ob_get_status()) {
        $requirements['output_buffering'] = [
          'title' => $this->t('Output Buffering'),
          'error_value' => $this->t('Not enabled'),
          'severity' => RequirementSeverity::Warning,
          'description' => $this->t('<a href="https://www.php.net/manual/en/function.ob-start.php">Output buffering</a> is not enabled. This may degrade Drupal\'s performance. You can enable output buffering by default <a href="https://www.php.net/manual/en/outcontrol.configuration.php#ini.output-buffering">in your PHP settings</a>.'),
        ];
      }
    }
    ...
    if ($phase == 'update') {
      ...
      $requirements['database_extensions'] = [
        'title' => $this->t('Database support'),
      ];

      // Make sure PDO is available.
      $database_ok = extension_loaded('pdo');
      if (!$database_ok) {
        $pdo_message = $this->t('Your web server does not appear to support PDO (PHP Data Objects). Ask your hosting provider if they support the native PDO extension. See the <a href=":link">system requirements</a> page for more information.', [
        ...
        if (empty(Database::getDriverList()->getInstallableList())) {
          $database_ok = FALSE;
          $pdo_message = $this->t('Your web server does not appear to support any common PDO database extensions. Check with your hosting provider to see if they support PDO (PHP Data Objects) and offer any databases that <a href=":drupal-databases">Drupal supports</a>.', [
        ...
        if (!defined('PDO::ATTR_DEFAULT_FETCH_MODE')) {
          $database_ok = FALSE;
          $pdo_message = $this->t('Your web server seems to have the wrong version of PDO installed. Drupal requires the PDO extension from PHP core. This system has the older PECL version. See the <a href=":link">system requirements</a> page for more information.', [
            ':link' => 'https://www.drupal.org/docs/system-requirements/php-requirements#database',
          ]);
        }
      }
      ...
      if (!$database_ok) {
        $requirements['database_extensions']['value'] = $this->t('Disabled');
        $requirements['database_extensions']['severity'] = RequirementSeverity::Error;
        $requirements['database_extensions']['description'] = $pdo_message;
      }
      else {
        $requirements['database_extensions']['value'] = $this->t('Enabled');
      }
    }
    ...
    $requirements['database_system'] = [
      'title' => $this->t('Database system'),
      'value' => $tasks->name(),
    ];
    $requirements['database_system_version'] = [
      'title' => $this->t('Database system version'),
    ...
    $requirements['database_support_json'] = [
      'title' => $this->t('Database support for JSON'),
      'severity' => RequirementSeverity::OK,
      'value' => $this->t('Available'),
      'description' => $this->t('Drupal requires databases that support JSON storage.'),
    ];

    if (!Database::getConnection()->hasJson()) {
      $requirements['database_support_json']['value'] = $this->t('Not available');
    ...
    $requirements['php_memory_limit'] = [
      ...
      'value' => $memory_limit == -1 ? $this->t('-1 (Unlimited)') : $memory_limit,
    ];

    if (!Environment::checkMemoryLimit(\Drupal::MINIMUM_PHP_MEMORY_LIMIT, $memory_limit)) {
      $description = [];
      if ($phase == 'update') {
        $description['phase'] = $this->t('Consider increasing your PHP memory limit to %memory_minimum_limit to help prevent errors in the update process.', ['%memory_minimum_limit' => \Drupal::MINIMUM_PHP_MEMORY_LIMIT]);
      }
      elseif ($phase == 'runtime') {
        $description['phase'] = $this->t('Depending on your configuration, Drupal can run with a %memory_limit PHP memory limit. However, a %memory_minimum_limit PHP memory limit or above is recommended, especially if your site uses additional custom or contributed modules.',
          [
      ...
      if (!empty($description['phase'])) {
        if ($php_ini_path = get_cfg_var('cfg_file_path')) {
          $description['memory'] = $this->t('Increase the memory limit by editing the memory_limit parameter in the file %configuration-file and then restart your web server (or contact your system administrator or hosting provider for assistance).', ['%configuration-file' => $php_ini_path]);
        }
        else {
          $description['memory'] = $this->t('Contact your system administrator or hosting provider for assistance with increasing your PHP memory limit.');
        }

        $handbook_link = $this->t('For more information, see the online handbook entry for <a href=":memory-limit">increasing the PHP memory limit</a>.', [':memory-limit' => 'https://www.drupal.org/node/207036']);
    ...
    if ($phase == 'runtime') {
      ...
      if (Settings::get('skip_permissions_hardening')) {
        $error_value = $this->t('Protection disabled');
        ...
        $error_value = $this->t('Not protected');
        ...
        if (!drupal_verify_install_file($site_path, FILE_NOT_WRITABLE, 'dir')) {
          $conf_errors[] = $this->t("The directory %file is not protected from modifications and poses a security risk. You must change the directory's permissions to be non-writable.", ['%file' => $site_path]);
        }
      }
      foreach (['settings.php', 'settings.local.php', 'services.yml'] as $conf_file) {
        ...
        if (file_exists($full_path) && !drupal_verify_install_file($full_path, FILE_EXIST | FILE_READABLE | FILE_NOT_WRITABLE, 'file', !Settings::get('skip_permissions_hardening'))) {
          $conf_errors[] = $this->t("The file %file is not protected from modifications and poses a security risk. You must change the file's permissions to be non-writable.", ['%file' => $full_path]);
        }
      }
      if (!empty($conf_errors)) {
        ...
        $requirements['configuration_files'] = [
          'value' => $this->t('Protected'),
        ];
      }
      $requirements['configuration_files']['title'] = $this->t('Configuration files');
    }
      ...
      foreach ($htaccessWriter->defaultProtectedDirs() as $protected_dir) {
        ...
        if (!file_exists($htaccess_file) || !($contents = @file_get_contents($htaccess_file)) || !str_contains($contents, 'Drupal_Security_Do_Not_Remove_See_SA_2013_003')) {
          ...
          $requirements[$htaccess_file] = [
            ...
            'value' => $this->t('Not fully protected'),
            'severity' => RequirementSeverity::Error,
            'description' => $this->t('See <a href=":url">@url</a> for information about the recommended .htaccess file which should be added to the %directory directory to help protect against arbitrary code execution.',
              [
                ':url' => $url,
                '@url' => $url,
                '%directory' => $protected_dir->getPath(),
              ]
            ),
          ];
        }
    ...
    if ($phase == 'runtime') {
      ...
      $summary = $this->t('Last run @time ago', ['@time' => \Drupal::service('date.formatter')->formatTimeDiffSince($cron_last)]);
      ...
      if ($severity != RequirementSeverity::Info) {
        $requirements['cron']['description'][] = [
          ...
          [
            '#markup' => $this->t('For more information, see the online handbook entry for <a href=":cron-handbook">configuring cron jobs</a>.', [':cron-handbook' => 'https://www.drupal.org/docs/administering-a-drupal-site/cron-automated-tasks/cron-automated-tasks-overview']),
            '#suffix' => ' ',
          ],
        ];
      }
    ...
    if (!empty($config_sync_directory)) {
      ...
      if (!is_dir($config_sync_directory)) {
        $description = $this->t('The directory %directory does not exist.', ['%directory' => $config_sync_directory]);
        $requirements['config sync directory'] = [
          'title' => $this->t('Configuration sync directory'),
          'description' => $description,
          'severity' => RequirementSeverity::Error,
        ];
      }
    ...
    if (empty($config_sync_directory)) {
      ...
      $requirements['config sync directory'] = [
        'title' => $this->t('Configuration sync directory'),
        'value' => $this->t('Not present'),
        'description' => $this->t("Your %file file must define the %setting setting as a string containing the directory in which configuration files can be found.",
          [
    ...
    foreach ($directories as $directory) {
      ...
      if (!$is_writable || !$is_directory) {
        $description = '';
        $requirements['file system']['value'] = $this->t('Not writable');
        if (!$is_directory) {
          $error = $this->t('The directory %directory does not exist.', ['%directory' => $directory]);
        }
        else {
          $error = $this->t('The directory %directory is not writable.', ['%directory' => $directory]);
        }
        // The files directory requirement check is done only during install and
        // runtime.
        if ($phase == 'runtime') {
          $description = $this->t('You may need to set the correct directory at the <a href=":admin-file-system">file system settings page</a> or change the current directory\'s permissions so that it is writable.', [':admin-file-system' => Url::fromRoute('system.file_system_settings')->toString()]);
        }
        ...
        // created.
        if (\Drupal::config('system.file')->get('default_scheme') == 'public') {
          $requirements['file system']['value'] = $this->t('Writable (<em>public</em> download method)');
        }
        else {
          $requirements['file system']['value'] = $this->t('Writable (<em>private</em> download method)');
        }
      }
    }
    ...
    if ($phase == 'runtime') {
      ...
      if ($has_pending_updates) {
        ...
        $requirements['update']['value'] = $this->t('Out of date');
        $requirements['update']['description'] = $this->t('Some modules have database schema updates to install. You should run the <a href=":update">database update script</a> immediately.', [':update' => Url::fromRoute('system.db_update')->toString()]);
      }

      $requirements['entity_update'] = [
        'title' => $this->t('Entity/field definitions'),
        'value' => $this->t('Up to date'),
      ];
    ...
    if ($phase == 'runtime') {
      if (Settings::get('update_free_access')) {
        $requirements['update access'] = [
          ...
          'severity' => RequirementSeverity::Error,
          'description' => $this->t('The update.php script is accessible to everyone without authentication check, which is a security risk. You must change the @settings_name value in your settings.php back to FALSE.', ['@settings_name' => '$settings[\'update_free_access\']']),
        ];
      }
      else {
        $requirements['update access'] = [
      ...
      $requirements['update access']['title'] = $this->t('Access to update.php');
    }
      ...
      foreach ($file->requires as $requirement) {
        ...
        if (!isset($files[$required_module])) {
          $requirements["$extension_name-$required_module"] = [
            'title' => $this->t('Unresolved dependency'),
            'description' => $this->t('@name requires this module.', ['@name' => $name]),
            'value' => $this->t('@required_name (Missing)', ['@required_name' => $required_module]),
            'severity' => RequirementSeverity::Error,
          ];
          continue;
        }
        ...
        if (!$requirement->isCompatible($version)) {
          $requirements["$extension_name-$required_module"] = [
            'title' => $this->t('Unresolved dependency'),
            'description' => $this->t('@name requires this module and version. Currently using @required_name version @version',
              [
                '@name' => $name,
            ...
            'value' => $this->t('@required_name (Version @compatibility required)',
              [
                '@required_name' => $required_name,
    ...
    if (!empty($removed_modules)) {
      ...
      foreach ($removed_modules as $removed_module) {
        $list[] = $this->t('<a href=":url">@module</a>', [
          ':url' => "https://www.drupal.org/project/$removed_module",
          '@module' => SystemRequirements::DRUPAL_CORE_REMOVED_MODULE_LIST[$removed_module],
        ]);
      }
    ...
    if (!empty($removed_themes)) {
      ...
      foreach ($removed_themes as $removed_theme) {
        $list[] = $this->t('<a href=":url">@theme</a>', [
          ':url' => "https://www.drupal.org/project/$removed_theme",
          '@theme' => SystemRequirements::DRUPAL_CORE_REMOVED_THEME_LIST[$removed_theme],
        ]);
      }
    ...
    $libraries = [
      Unicode::STATUS_SINGLEBYTE => $this->t('Standard PHP'),
      Unicode::STATUS_MULTIBYTE => $this->t('PHP Mbstring Extension'),
      Unicode::STATUS_ERROR => $this->t('Error'),
    ];
    $severities = [
    ...
    $requirements['unicode'] = [
      'title' => $this->t('Unicode library'),
      'value' => $libraries[$library],
      'severity' => $severities[$library],
    ];
    switch ($failed_check) {
      case 'mb_strlen':
        $requirements['unicode']['description'] = $this->t('Operations on Unicode strings are emulated on a best-effort basis. Install the <a href="http://php.net/mbstring">PHP mbstring extension</a> for improved Unicode support.');
        break;

      case 'mbstring.encoding_translation':
        $requirements['unicode']['description'] = $this->t('Multibyte string input conversion in PHP is active and must be disabled. Check the php.ini <em>mbstring.encoding_translation</em> setting. Refer to the <a href="http://php.net/mbstring">PHP mbstring documentation</a> for more information.');
        break;
    }
    ...
    if ($phase == 'runtime') {
      ...
      if (!\Drupal::moduleHandler()->moduleExists('update')) {
        $requirements['update status'] = [
          'value' => $this->t('Not enabled'),
          'severity' => RequirementSeverity::Warning,
          'description' => $this->t('Update notifications are not enabled. It is <strong>highly recommended</strong> that you install the Update Status module from the <a href=":module">module administration page</a> in order to stay up-to-date on new releases. For more information, <a href=":update">Update status handbook page</a>.', [
      ...
      $requirements['update status']['title'] = $this->t('Update notifications');

      if (Settings::get('rebuild_access')) {
        $requirements['rebuild access'] = [
          'title' => $this->t('Rebuild access'),
          'value' => $this->t('Enabled'),
          'severity' => RequirementSeverity::Error,
    ...
    if ($phase === 'runtime' && PHP_SAPI !== 'cli') {
      $samesite = ini_get('session.cookie_samesite') ?: $this->t('Not set');
      ...
      $requirements['php_session_samesite'] = [
        'title' => $this->t('SameSite cookie attribute'),
        'value' => $samesite,
        'severity' => $valid ? RequirementSeverity::OK : RequirementSeverity::Warning,
        'description' => $this->t('This attribute should be explicitly set to Lax, Strict or None. If set to None then the request must be made via HTTPS. See <a href=":url" target="_blank">PHP documentation</a>', [
    ...
    if ($phase == 'runtime') {
      ...
      if (empty($trusted_host_patterns)) {
        $requirements['trusted_host_patterns'] = [
          'title' => $this->t('Trusted Host Settings'),
          'value' => $this->t('Not enabled'),
          'description' => $this->t('The trusted_host_patterns setting is not configured in settings.php. This can lead to security vulnerabilities. It is <strong>highly recommended</strong> that you configure this. See <a href=":url">Protecting against HTTP HOST Header attacks</a> for more information.', [':url' => 'https://www.drupal.org/docs/installing-drupal/trusted-host-settings']),
          'severity' => RequirementSeverity::Error,
        ];
      }
      else {
        $requirements['trusted_host_patterns'] = [
          'title' => $this->t('Trusted Host Settings'),
          'value' => $this->t('Enabled'),
          'description' => $this->t('The trusted_host_patterns setting is set to allow %trusted_host_patterns', ['%trusted_host_patterns' => implode(', ', $trusted_host_patterns)]),
        ];
      }
    }
      ...
      if (str_contains($autoload, 'src/Driver/Database/')) {
        ...
        if (!in_array('enable_provider_database_driver', array_keys($pending_updates['system']['pending'] ?? []), TRUE)) {
          ...
          $requirements['database_driver_provided_by_module'] = [
            'title' => $this->t('Database driver provided by module'),
            'value' => $this->t('Not installed'),
            'description' => $this->t('The current database driver is provided by the module: %module. The module is currently not installed. You should immediately <a href=":install">install</a> the module.',
              [
                '%module' => $provider,
    ...
    if (extension_loaded('xdebug')) {
      ...
      if ($current_nesting_level < $minimum_nesting_level) {
        $requirements['xdebug_max_nesting_level'] = [
          'title' => $this->t('Xdebug settings'),
          'value' => $this->t('xdebug.max_nesting_level is set to %value.', ['%value' => $current_nesting_level]),
          'description' => $this->t('Set <code>xdebug.max_nesting_level=@level</code> in your PHP configuration as some pages in your Drupal site will not work when this setting is too low.', ['@level' => $minimum_nesting_level]),
          'severity' => RequirementSeverity::Error,
        ];
      }
    }
    ...
    if (str_starts_with(PHP_OS, 'WIN')) {
      ...
      if ($depth > 120) {
        $requirements['max_path_on_windows'] = [
          'title' => $this->t('Windows installation depth'),
          'description' => $this->t('The public files directory path is %depth characters. Paths longer than 120 characters will cause problems on Windows.', ['%depth' => $depth]),
          'severity' => RequirementSeverity::Error,
        ];
      }
    }
    ...
    if (PHP_INT_SIZE <= 4) {
      $requirements['limited_date_range'] = [
        'title' => $this->t('Limited date range'),
        'value' => $this->t('Your PHP installation has a limited date range.'),
    ...
    if ($phase === 'runtime') {
      ...
      if (array_key_exists('install_profile', $settings)) {
        ...
        $requirements['install_profile_in_settings'] = [
          'title' => $this->t('Install profile in settings'),
          'value' => $this->t("Drupal 9 no longer uses the \$settings['install_profile'] value in settings.php and it should be removed."),
          'severity' => RequirementSeverity::Warning,
        ];
      }
    }
    ...
    if ($phase == 'update') {
      ...
      if (isset($module_list['user'])) {
        $requirements['user_update_last_removed'] = [
          'title' => $this->t('The version of Drupal you are trying to update from is too old'),
          'description' => $this->t('Updating to Drupal @current_major is only supported from Drupal version @required_min_version or higher. If you are trying to update from an older version, first update to the latest version of Drupal @previous_major. (<a href=":url">Drupal upgrade guide</a>)', [
            '@current_major' => 10,
        ...
        foreach ($module_list as $module => $data) {
          $requirements[$module . '_update_last_removed'] = [
            'title' => $this->t('Unsupported schema version: @module', ['@module' => $data['name']]),
            'description' => $this->t('The installed version of the %module module is too old to update. Update to an intermediate version first (last removed version: @last_removed_version, installed version: @installed_version).', [
              '%module' => $data['name'],
        ...
        foreach ($modules as $module => $extension) {
          ...
          if ($missing_updates = array_diff(array_keys($removed_post_updates), $existing_updates)) {
            ...
            $requirements[$module . '_post_update_removed'] = [
              'title' => $this->t('Missing updates for: @module', ['@module' => $module_info->info['name']]),
              'description' => $description,
              'severity' => RequirementSeverity::Error,
            ];
          }
          ...
          foreach ($equivalent_updates as $future_update => $data) {
            ...
            if (!function_exists($ran_update_function_name) && !function_exists($future_update_function_name)) {
              ...
              $requirements[$module . '_equivalent_update_missing'] = [
                'title' => $this->t('Missing updates for: @module', ['@module' => $module_info->info['name']]),
                'description' => $this->t('The version of the %module module that you are attempting to update to is missing update @future_update (which was marked as an equivalent by @ran_update). Update to at least @future_version_string.', [
                  '%module' => $module_info->info['name'],
    ...
    if ($phase === 'runtime') {
      ...
      if ($twig_debug || $twig_cache_disable) {
        $requirements['twig_debug_enabled'] = [
          ...
          'value' => $this->t('Twig development mode settings are turned on. Go to @link to disable them.', [
      ...
      if ($render_cache_disabled) {
        $requirements['render_cache_disabled'] = [
          'title' => $this->t('Markup caching disabled'),
          'value' => $this->t('Render cache, dynamic page cache, and page cache are bypassed. Go to @link to enable them.', [
            '@link' => Link::createFromRoute(
              'development settings page',
              'system.development_settings',
            )->toString(),
