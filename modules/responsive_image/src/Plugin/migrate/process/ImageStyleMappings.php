...
class ImageStyleMappings extends ProcessPluginBase {
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    $mappings = $row->getSourceProperty('mapping');
    $breakpoint_group = $row->getSourceProperty('breakpoint_group');
    foreach ($mappings as $mapping_id => $mapping) {
      ...
      // the last period.
      preg_match('/\.([a-z]+)$/', $mapping_id, $matches);
   ...
   *   The multiplier settings.
   */
  protected function getMultiplierSettings($multiplier_settings) {
    $settings = [];
